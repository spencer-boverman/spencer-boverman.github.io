...
function begin_play(){
	...
	if(game_started()){
		alert("Game is already started. Please Reset Play to play again.");
	}else{
		...
		if(isEmpty(document.getElementById("player1_id").value)  || isEmpty(document.getElementById("player2_id").value)){
			...
			document.getElementById("turn_info").innerHTML = document.getElementById("player1_id").value.bold() + " moves next"
		}
	}
}
...
function reset_play(){
	...
	for(var i = 0; i < board_state.length; i++){
		...
		var boardCoord = table_ids[i];
		document.getElementById(boardCoord).innerHTML = boardCoord;
...
function play() {
	...
	var moveCheckRegex = /^[A-C][1-3]$/g;

	var moveInput = document.getElementById("move_text_id").value;

	var moveIndex = table_ids.indexOf(moveInput);

	if(!game_started()){
		alert("The game has not started.");
	} else if(moveCheckRegex.test(moveInput) && (moveInput == document.getElementById(moveInput).innerHTML)){
		...
		if(whose_move()){
			...
			document.getElementById("turn_info").innerHTML = document.getElementById("player2_id").value.bold() + " moves next"
		}
